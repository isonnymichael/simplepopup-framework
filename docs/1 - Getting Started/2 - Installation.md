# How to Install SimplePopup Framework?
...
One of the problem when working with Plugins in WordPress is that, you can't have the same namespace across multiple plugin. e.g `SimplePopup\WordPress`.
...
- To refactor this framework you can run : `aspri --wp-refactor --path {pathtoproject} --from SimplePopup --to {Projectnamespace} --type {plugin|theme}`
    - or via grunt : `grunt shell:simplepopup_refactor` (you need to set the `{Projectnamespace}` in `Gruntfile.js`)
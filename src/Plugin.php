...
class Plugin
{
	...
	public function loadModels()
	{
		...
		foreach ($models as $model) {
			...
			foreach ($model->getHooks() as $hook) {
				$class = str_replace('SimplePopup\\WordPress\\Hook\\', '', get_class($hook));
	...
	private function loadFeatures()
	{
		...
		foreach ($features as $feature) {
			...
			$feature = '\\SimplePopup\\Feature\\' . $name;
		...
		foreach ($controllers as $controller) {
			...
			foreach ($controller->getHooks() as $hook) {
				...
				if (in_array(strtolower($class), $this->enableHooks)) {
					...
					$status = isset($this->config->options->simplepopup_hooks->$key)
						? $this->config->options->simplepopup_hooks->$key
	...
	public function setDefaultOption()
	{
		...
		if (empty($config) || !$config) {
			...
			$this->WP->update_option('simplepopup_config', (object) $config);

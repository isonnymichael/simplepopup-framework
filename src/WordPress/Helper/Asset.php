...
namespace SimplePopup\WordPress\Helper;
 ...
 *
 * @package    SimplePopup
 * @subpackage SimplePopup\WordPress
 */
trait Asset
{
	/**
	...
	public function wp_enqueue_style(
		...
		if (!strpos($src, '//')) {
			$file = sprintf(
				'%sassets/%s',
				json_decode(SIMPLEPOPUP_PATH)->framework_path,
				$src
			);
			$src = sprintf('%sassets/%s', json_decode(SIMPLEPOPUP_PATH)->framework_url, $src);
			if (!file_exists($file)) {
				return;
			}
		}
